...
# SCPN ICF Impact Core — diagnostic and clock semantics tests
...
from scpn_icf_impact_core.errors import DiagnosticPlanError
...
EVENT_BINDINGS = {
    ...
    "event_reference": "synthetic impact fiducial marker",
    "provenance": "synthetic fixture",
...
REFERENCE_FRAMES = (
    ...
    ReferenceFrame(
        identifier="frm_launcher",
        kind=FrameKind.BEAMLINE,
        description="projectile-launcher line-of-flight frame",
    ),
)
CLOCK_RELATIONS = (
    ClockRelation(
...
SIGNALS_CH_IMPACT_TIMING_TRAIN = (
    SignalDeclaration(
        identifier="sig_impact_cycle",
    ...
    SignalDeclaration(
        identifier="sig_impact_duration",
        quantity="time",
        unit="s",
        role=SignalRole.AUXILIARY,
    ...
    SignalDeclaration(
        identifier="sig_projectile_speed",
        quantity="velocity",
        unit="m/s",
        role=SignalRole.AMPLITUDE,
        description="synthetic projectile arrival speed",
    ),
)
...
REFERENCE_TRANSFORMATIONS: tuple[FrameTransformation, ...] = (
    FrameTransformation(
        ...
        target_identifier="frm_launcher",
...
def channel_event_train() -> DiagnosticChannelPlan:
    """Build the synthetic projectile-arrival event channel."""
    return DiagnosticChannelPlan(
        identifier="ch_impact_timing_train",
        ...
        signals=SIGNALS_CH_IMPACT_TIMING_TRAIN,
        synthetic=True,
    )
...
def channel_derived() -> DiagnosticChannelPlan:
    """Build the synthetic impact-asymmetry mode channel."""
    return DiagnosticChannelPlan(
...
def synthetic_plan() -> DiagnosticPlan:
    ...
    return DiagnosticPlan(
        identifier="icf_impact_reference_plan",
...
def test_candidate_rejects_malformed_identifier() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="candidate_id"):
        CandidateProfile(
            candidate_id="Bad-Id",
            phenomenon="x",
            configurations=("projectile_or_impact_icf",),
...
def test_candidate_rejects_unsorted_configurations() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="unique and sorted"):
        CandidateProfile(
            candidate_id="a.b",
            phenomenon="x",
            configurations=("projectile_or_impact_icf", "projectile_or_impact_icf"),
            observability_class=ObservabilityClass.NONCYCLIC_FEATURE,
        )
...
def test_transformation_rejects_self_mapping() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="to itself"):
        _transformation(target_identifier="frm_chamber")
    ...
    with pytest.raises(DiagnosticPlanError, match="must be sorted"):
        _plan_with(
            ...
            frame_transformations=(
                _transformation(
                    source_identifier="frm_launcher",
    ...
    with pytest.raises(DiagnosticPlanError, match="duplicate transformation pair"):
        _plan_with(
            frame_transformations=(
                _transformation(),
                _transformation(
                    source_identifier="frm_launcher", target_identifier="frm_chamber"
                ),
            )
        )
...
def test_plan_accepts_explicit_deferral() -> None:
    ...
    plan = DiagnosticPlan(
        identifier="icf_impact_partial_plan",
        binding=CATALOGUE_BINDING,
...
def test_report_flags_mhd_band_outside_typical_range() -> None:
    ...
    findings = plan.consistency_report()
    assert len(findings) == 1
    assert "LA-8000-C" in findings[0].message
...
def test_report_flags_window_beyond_device_ceiling() -> None:
    ...
    channel = _derived(acquisition_duration_s=0.0001)
    plan = synthetic_plan()
    plan = DiagnosticPlan(
        identifier=plan.identifier,
        binding=plan.binding,
