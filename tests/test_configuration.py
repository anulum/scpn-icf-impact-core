...
# SCPN ICF Impact Core — device configuration container tests
...
def synthetic_configuration(
    identifier: str = "projectile_or_impact_icf",
    velocity_km_s: float = 150.0,
) -> DeviceConfiguration:
    """Build a valid synthetic configuration with optional overrides."""
    return DeviceConfiguration(
        identifier=identifier,
        projectile=Projectile(mass_mg=10.0, velocity_km_s=velocity_km_s),
        target=TargetDeclaration(target_radius_um=3000.0),
        registry=REGISTRY,
    )
...
def test_owned_identifier_constructs() -> None:
    """The owned identifier constructs with an in-scale projectile."""
    assert synthetic_configuration().identifier == "projectile_or_impact_icf"
...
def test_consistency_report_clean_and_finding() -> None:
    """The report is empty at scale and precise below it."""
    assert synthetic_configuration().consistency_report() == ()
    slow = synthetic_configuration(velocity_km_s=10.0)
    findings = slow.consistency_report()
    assert len(findings) == 1
    assert "entry scale" in findings[0].message
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda r: {**r, "projectile": None}, "projectile: must be an object"),
...
def test_from_record_field_type_violations() -> None:
    ...
    record = synthetic_configuration().to_record()
    record["projectile"]["mass_mg"] = "big"
    with pytest.raises(DeviceConfigurationError, match="mass_mg: must be"):
        configuration_from_record(record)
    record = synthetic_configuration().to_record()
    record["projectile"]["velocity_km_s"] = True
    with pytest.raises(DeviceConfigurationError, match="velocity_km_s: must be"):
        configuration_from_record(record)
    record = synthetic_configuration().to_record()
...
def test_integer_accepted_where_number_expected() -> None:
    ...
    record = synthetic_configuration().to_record()
    record["projectile"]["velocity_km_s"] = 150
    restored = configuration_from_record(record)
    assert restored.projectile.velocity_km_s == 150.0
