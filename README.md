...
inertial confinement fusion systems within the SCPN Reactor Systems
Research Group. This repository is the designated owner of device-level
truth for the `projectile_or_impact_icf` configuration of the SCPN Phase
Orchestrator reactor registry (impact-driven target).
...
physics — what a flying plate carries onto the target it strikes, what
the fuel is in each of the two target geometries a freely published
workshop proceedings describes, and where that fuel ends up when it is
compressed (ADR 0005, evidence:
`VALIDATION.md#level-0-device-physics`); and the two geometry tiers —
the tessellated and B-rep models of **both** published schemes on the
shared kernel library, each in its own frame, because no filed source
pairs them (ADR 0006, evidence: `VALIDATION.md#device-3d-model` and
...
This repository owns, for the impact-ICF device family:

- the device boundary: plant and experiment truth, shot lifecycle, and
  configuration policy for fusion schemes in which a macroscopic
  projectile, accelerated to hypervelocity, converts its kinetic energy on
  impact into the compression and heating of a fuel-bearing target;
- launcher-and-impact semantics at device level: launcher-class
  declarations (electromagnetic, light-gas, or staged launchers),
  projectile metrology and integrity contracts, impact-geometry and
  timing declarations, and kinetic-to-internal energy conversion
  declarations;
...
- **Laser-driven ICF**: `SCPN-ICF-LASER-CORE`. **Beam-driven ICF**:
  `SCPN-ICF-BEAM-CORE`.
- **Mechanical and liquid-liner magnetised-target fusion** (slower liner
  compression of a magnetised plasma): `SCPN-MIF-LINER-CORE`.
...
artefact, and no parameter set describes or validates any real machine. Launcher-class and fuel choices are configuration
